...
export class OpenAIWebSearchScraper {
    ...
    const messages = [
      {
        ...
        content:
          `You are a job search assistant. Search for ${input.search_term} positions using web search capabilities. Focus on recent job postings from reputable job boards.`,
      },
      {
        role: "user" as const,
        content: `Find ${
  ...
  private buildSearchQuery(input: ScraperInput): string {
    ...
    query +=
      " jobs site:indeed.com OR site:linkedin.com OR site:glassdoor.com OR site:monster.com";
    ...
    const mockResults: OpenAISearchResult[] = [
      {
        ...
        snippet:
          `We are looking for an experienced ${query} developer to join our team. Remote work available.`,
      ...
      },
      {
        ...
        snippet:
          `Join our innovative team as a ${query} engineer. Competitive salary and benefits.`,
  ...
  private convertSearchResultToJob(
    ...
    try {
      ...
      const company = this.extractCompanyFromSnippet(result.snippet) ||
        "Unknown Company";
      const location = this.extractLocationFromSnippet(result.snippet) ||
