 ...
 */
import { CompensationInterval, DescriptionFormat, getCountryDomain, JobType, Scraper, Site, } from '../../types/scrapers.js';
...
export class IndeedScraper extends Scraper {
    ...
    async scrape(scraper_input) {
        ...
        let page = 1;
        let cursor = null;
        while (this.seen_urls.size <
            (scraper_input.results_wanted ?? 15) + (scraper_input.offset ?? 0)) {
            console.log(`search page: ${page} / ${Math.ceil(((scraper_input.results_wanted ?? 15) + (scraper_input.offset ?? 0)) / this.jobs_per_page)}`);
    ...
    async _scrape_page(cursor) {
        ...
        const query = JOB_SEARCH_QUERY.replace('{what}', search_term ? `what: "${search_term}"` : '')
            .replace('{location}', this.scraper_input?.location
            ? `location: {where: "${this.scraper_input.location}", radius: ${this.scraper_input.distance ?? 25}, radiusUnit: MILES}`
            : '')
        ...
        try {
            ...
            if (!response.ok) {
                console.log(`❌ Indeed API error with status code: ${response.status} (submit GitHub issue if this appears to be a bug)`);
    ...
    async _get_next_cursor(current_cursor) {
        if (!this.scraper_input)
            return null;
        const filters = this._build_filters();
        const search_term = this.scraper_input.search_term
    ...
    _get_compensation(compensation) {
    ...
    _is_job_remote(job, description) {
        const remote_keywords = ['remote', 'work from home', 'wfh'];
        const is_remote_in_attributes = job.attributes.some((attr) => remote_keywords.some((keyword) => attr.label.toLowerCase().includes(keyword)));
        const is_remote_in_description = remote_keywords.some((keyword) => description.toLowerCase().includes(keyword));
        const is_remote_in_location = job.location.formatted?.long &&
            remote_keywords.some((keyword) => job.location.formatted.long.toLowerCase().includes(keyword));
        return is_remote_in_attributes || is_remote_in_description || is_remote_in_location || null;
    }
    _get_compensation_interval(interval) {
        if (!interval)
            return null;
    ...
    _get_enum_from_job_type(job_type_str) {
    ...
    _markdown_converter(description_html) {
    ...
    _plain_converter(description_html) {
        return description_html
            .replace(/<[^>]*>/g, '')
            .replace(/&nbsp;/g, ' ')
            .replace(/&amp;/g, '&')
    ...
    _extract_emails_from_text(text) {
        if (!text)
            return null;
