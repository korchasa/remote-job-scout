...
export class FilteringService {
     ...
     */
    filterVacancies(vacancies, settings) {
        const result = {
            success: true,
            filteredVacancies: [],
            skippedVacancies: [],
        ...
        try {
            ...
            console.log(`✅ Filtering completed: ${result.filteredCount} passed, ${result.skippedCount} skipped`);
     ...
     */
    shouldIncludeVacancy(vacancy, settings) {
     ...
     */
    isCompanyBlacklisted(vacancy, blacklistedCompanies) {
        if (!blacklistedCompanies || blacklistedCompanies.length === 0) {
            return false;
        }
        const vacancyData = this.parseVacancyData(vacancy);
        const companyName = typeof vacancyData.company === 'string' ? vacancyData.company.toLowerCase() : '';
        return blacklistedCompanies.some((blacklisted) => companyName.includes(blacklisted.toLowerCase()));
    }
    /**
     * Проверяет наличие запрещенных слов в тексте
     */
    containsBlacklistedWords(text, blacklistedWords) {
     ...
     */
    matchesCountryFilter(vacancy, countryFilters) {
     ...
     */
    matchesLanguageRequirements(vacancy, languageRequirements) {
        ...
        const hasEnglishRequirement = languageRequirements.some((req) => req.language.toLowerCase() === 'english');
