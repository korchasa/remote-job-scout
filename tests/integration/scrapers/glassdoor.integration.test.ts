/**
 * Интеграционные тесты для Glassdoor скрейпера - реальные запросы к Glassdoor GraphQL API
 *
 * Эти тесты делают реальные HTTP запросы к Glassdoor API
...
import { GlassdoorScraper } from '../../../src/services/scrapers/glassdoor.ts';

/**
 * Тест реального скрейпинга Glassdoor с базовым поиском
 */
test('Glassdoor Integration - Basic job search with real API', async () => {
  ...
  console.log('🔍 Starting real Glassdoor API integration test...');
  ...
  for (const job of result.jobs) {
    ...
    expect(job.job_url.includes('glassdoor.com')).toBe(true); // 'URL должен содержать glassdoor.com'
  ...
  console.log(`🎯 Successfully scraped ${result.jobs.length} jobs from Glassdoor API`);
});

/**
 * Тест поиска удаленных вакансий с реальным API
 */
test('Glassdoor Integration - Remote jobs search', async () => {
  const scraper = new GlassdoorScraper();
 ...
 */
test('Glassdoor Integration - Job type filtering', async () => {
  const scraper = new GlassdoorScraper();
 ...
 */
test('Glassdoor Integration - Multi-country search', async () => {
  const scraper = new GlassdoorScraper();
    ...
    try {
      ...
      if (result.jobs.length > 0) {
        ...
        const countryDomains: Record<string, string> = {
          [Country.USA]: 'glassdoor.com',
          [Country.UK]: 'glassdoor.co.uk',
          [Country.CANADA]: 'glassdoor.ca',
        };
 ...
 */
test('Glassdoor Integration - Easy apply jobs', async () => {
  const scraper = new GlassdoorScraper();
 ...
 */
test('Glassdoor Integration - Large result set with pagination', async () => {
  const scraper = new GlassdoorScraper();
 ...
 */
test('Glassdoor Integration - Complex search with multiple filters', async () => {
  const scraper = new GlassdoorScraper();
 ...
 */
test('Glassdoor Integration - Error handling', async () => {
  const scraper = new GlassdoorScraper();
 ...
 */
test('Glassdoor Integration - Performance test', async () => {
...
// Test Glassdoor scraper with JobSpy compatible interface
integrationTest('GlassdoorScraper - GraphQL API integration (from unit tests)', async () => {
  const scraper = new GlassdoorScraper();
  ...
  console.log('Testing Glassdoor scraper with JobSpy compatible interface...');
  ...
  try {
    ...
    if (result.jobs.length > 0) {
      ...
      console.log(`Successfully scraped ${result.jobs.length} jobs from Glassdoor`);
...
integrationTest('GlassdoorScraper - remote jobs search (from unit tests)', async () => {
  const scraper = new GlassdoorScraper();
...
integrationTest('GlassdoorScraper - job type filtering (from unit tests)', async () => {
  const scraper = new GlassdoorScraper();
...
integrationTest('GlassdoorScraper - country localization (from unit tests)', async () => {
  const scraper = new GlassdoorScraper();
...
integrationTest('GlassdoorScraper - description formats (from unit tests)', async () => {
  const scraper = new GlassdoorScraper();
...
integrationTest('GlassdoorScraper - hours old filtering (from unit tests)', async () => {
  const scraper = new GlassdoorScraper();
...
integrationTest('GlassdoorScraper - easy apply filtering (from unit tests)', async () => {
  const scraper = new GlassdoorScraper();
...
integrationTest('GlassdoorScraper - pagination and offset (from unit tests)', async () => {
  const scraper = new GlassdoorScraper();
...
integrationTest('GlassdoorScraper - comprehensive job data (from unit tests)', async () => {
  const scraper = new GlassdoorScraper();
...
integrationTest('GlassdoorScraper - multiple countries (from unit tests)', async () => {
  const scraper = new GlassdoorScraper();
  const countries = [Country.USA, Country.UK, Country.CANADA];
...
integrationTest('GlassdoorScraper - empty search term (from unit tests)', async () => {
  const scraper = new GlassdoorScraper();
...
integrationTest('GlassdoorScraper - JobSpy compatibility test (from unit tests)', async () => {
  const scraper = new GlassdoorScraper();
